        ...
        <tbody>
            ...
            if (mysqli_num_rows($result_jidla) > 0) {
                while ($row = mysqli_fetch_assoc($result_jidla)) {
                    ...
                    echo "<td><form action='".$_SERVER['PHP_SELF']."' method='post'><input type='hidden' name='id_jidla' value='".$row['ID_jidla']."'><input type='hidden' name='atribut' value='název'><input type='text' name='nova_hodnota' value='" . $row['název'] . "'><input type='submit' value='Uložit'></form></td>";
                    echo "<td><form action='".$_SERVER['PHP_SELF']."' method='post'><input type='hidden' name='id_jidla' value='".$row['ID_jidla']."'><input type='hidden' name='atribut' value='typ'><select name='nova_hodnota'><option value='Bagety'" . ($row['typ'] == 'Bagety' ? ' selected' : '') . ">Bagety</option><option value='Chlebíčky'" . ($row['typ'] == 'Chlebíčky' ? ' selected' : '') . ">Chlebíčky</option><option value='Kaiserky'" . ($row['typ'] == 'Kaiserky' ? ' selected' : '') . ">Kaiserky</option><option value='Croissanty'" . ($row['typ'] == 'Croissanty' ? ' selected' : '') . ">Croissanty</option><option value='Dezerty'" . ($row['typ'] == 'Dezerty' ? ' selected' : '') . ">Dezerty</option></select><input type='submit' value='Uložit'></form></td>";
                    echo "<td><form action='".$_SERVER['PHP_SELF']."' method='post'><input type='hidden' name='id_jidla' value='".$row['ID_jidla']."'><input type='hidden' name='atribut' value='cena'><input type='number' name='nova_hodnota' value='" . $row['cena'] . "'><input type='submit' value='Uložit'></form></td>";
                    echo "<td><form action='".$_SERVER['PHP_SELF']."' method='post'><input type='hidden' name='id_jidla' value='".$row['ID_jidla']."'><input type='hidden' name='atribut' value='popis'><input type='text' name='nova_hodnota' value='" . $row['popis'] . "'><input type='submit' value='Uložit'></form></td>";
                    echo "<td><form action='".$_SERVER['PHP_SELF']."' method='post' enctype='multipart/form-data'><input type='hidden' name='id_jidla' value='".$row['ID_jidla']."'><input type='hidden' name='atribut' value='img'><input type='file' name='obrazek' accept='image/*'><input type='submit' value='Nahrát nový obrázek'></form><img src='" . $obrazky_adresar . $row['img'] . "' alt='" . $row['název'] . "' style='width:100px;height:100px;'></td>";
                    echo "<td><form action='".$_SERVER['PHP_SELF']."' method='post'><input type='hidden' name='id_jidla' value='".$row['ID_jidla']."'><input type='hidden' name='atribut' value='delete'><input type='submit' value='Odstranit'></form></td>";
                    echo "</tr>";
                }
            } else {
