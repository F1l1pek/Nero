...
// Dotaz na košík pro konkrétního uživatele
...
function provedObjednavku($ID_U, $typ_uzivatele) {
    ...
    $insert_order_query = "INSERT INTO obednavky (datum, ID_U, stav) VALUES (CURRENT_TIMESTAMP(), ?, ?)";
    $stmt = $dbSpojeni->prepare($insert_order_query);
    $stmt->bind_param("is", $ID_U, $stav);
    $stmt->execute();

    // Získání ID nově vytvořené objednávky
    $idObjednavky = $stmt->insert_id;
    ...
    foreach ($polozky as $polozka) {
        ...
        $sql_cena = "SELECT cena, cena_s, cena_f FROM jidla WHERE ID_jidla = ?";
        $stmt_cena = $dbSpojeni->prepare($sql_cena);
        $stmt_cena->bind_param("i", $idJidla);
        $stmt_cena->execute();
        $result_cena = $stmt_cena->get_result();
        $radek_cena = $result_cena->fetch_assoc();
        ...
        $insert_order_item_query = "INSERT INTO obednavky_produkty (ID_O, ID_J, mnozstvi, cena) VALUES (?, ?, ?, ?)";
        $stmt_item = $dbSpojeni->prepare($insert_order_item_query);
        $stmt_item->bind_param("iiid", $idObjednavky, $idJidla, $mnozstvi, $cena);
        $stmt_item->execute();
    }
    // Vymazání košíku pro uživatele
    $delete_cart_query = "DELETE FROM košik WHERE ID_U = ?";
    $stmt_delete = $dbSpojeni->prepare($delete_cart_query);
    $stmt_delete->bind_param("i", $ID_U);
    $stmt_delete->execute();
}
?>
        ...
        <form method="post">
            <table>
                ...
                foreach ($polozky as $polozka):
                    ...
                    if ($typ_uzivatele == "admin" || $typ_uzivatele == "běžný") {
                        $subtotal = $polozka['cena'] * $polozka['mnozstvi'];
                    }
                    if ($typ_uzivatele == "velkoodberatel_s") {
                        $subtotal = $polozka['cena_s'] * $polozka['mnozstvi'];
                    }
                    if ($typ_uzivatele == "velkoodberatel_f") {
                        $subtotal = $polozka['cena_f'] * $polozka['mnozstvi'];
                    }
                ...
                ?>
                    <tr>
                        ...
                        <?php if ($typ_uzivatele == "admin" || $typ_uzivatele == "běžný"): ?>
                            <td><?php echo $polozka['cena']; ?> Kč</td>
                        <?php endif; ?>
                        <?php if ($typ_uzivatele == "velkoodberatel_s"): ?>
                            <td><?php echo $polozka['cena_s']; ?> Kč</td>
                        <?php endif; ?>
                        <?php if ($typ_uzivatele == "velkoodberatel_f"): ?>
                            <td><?php echo $polozka['cena_f']; ?> Kč</td>
                        <?php endif; ?>
                        <td>
                            <!-- Tlačítka pro přidání a odebrání množství -->
                            <button type="button" aria-label="Decrease" onclick="decrement(<?php echo $polozka['ID_jidla']; ?>, <?php echo $polozka['ID_O']; ?>)" class="ikonka">-</button>
                            ...
                            <button type="button" aria-label="Increase" onclick="increment(<?php echo $polozka['ID_jidla']; ?>, <?php echo $polozka['ID_O']; ?>)" class="ikonka">+</button>
                        </td>
